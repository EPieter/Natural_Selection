...
class Store(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.image.blit(text, text_rect)
        text = font.render(data.tr.get('Production') + ' ' + data.ToolStore.moneySymbol() + '/sp', True, data.WHITE_TEXT)
        ...
        for i in range(1):
            for j in range(7):
                ...
                price = sprites.menu_items[j][2] if sprites.menu_items[j][0] != "Bitcoin" else float(game.bitcoin_price)
                text = font.render(data.ToolStore.moneySymbol() + ' ' + data.ToolStore.priceFormatter(price), True, data.WHITE_TEXT)
                text_rect = (10 + 1.2 * 76, 68 + j * 76, 500, 24)
                self.image.blit(text, text_rect)
                ...
                self.image.blit(text, text_rect)
                text = font.render(str(data.ToolStore.priceFormatter(sprites.menu_items[j][4], ",.3f")), True, data.WHITE_TEXT) if sprites.menu_items[j][4] != 0 else font.render("n/a", True, data.WHITE_TEXT)
